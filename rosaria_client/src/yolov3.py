...
import rospy
import rospy
from rosaria_client.msg import BoundingBox
...
class InferenceModel(OAKSingleStageModel):
    def __init__(
        ...
        **kwargs,
        ...
        self.pub = rospy.Publisher('BoundingBox', BoundingBox, queue_size=100)
        ...
        for result, input_info in zip(predictions[0], predictions[1]):
            ...
            for box, conf, class_id in zip(boxes, confidences, class_ids):
                ...
                if class_id == 1:
                    ...
                    y2=int(np.clip(box[3], 0, original_h))
                    score=float(conf)
                    boundingbox = BoundingBox(probability=score, xmin=x1, ymin=y1, xmax=x2, ymax=y2)
